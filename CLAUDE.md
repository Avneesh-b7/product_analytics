...
An eleventh lab in `correl_analysis/correl_analysis.ipynb` — correlation analysis and activation-based risk segmentation on a synthetic GrowthTech SaaS dataset (`data/growthtech_activation_retention_data.xlsx`, 1,200 users, gitignored, one row per user).

Columns: `user_id`, `signup_date`, `acquisition_channel`, four binary activation flags (`account_setup_completed`, `first_project_created`, `team_invite_sent`, `premium_trial_started`), `total_sessions`, and two binary outcomes (`retained_30_day`, `retained_90_day`).

Key design decisions:

- **Coefficient choice**: Pearson (`.corr()` / `.corrwith()`) for the binary activation flags against the binary retention outcomes — this reduces to the point-biserial coefficient, the natural default for binary×binary or binary×continuous pairs. `total_sessions` is right-skewed (long tail of highly engaged users, confirmed via histogram + KDE overlay and `.describe()`), so **Spearman is treated as the primary coefficient for `total_sessions`** since it only requires a monotonic relationship and isn't distorted by skew/outliers; Pearson is still reported alongside as a sanity check. Both coefficients are computed for every variable as a matter of course.
- **Significance testing**: every coefficient is tested against H₀: ρ = 0 via `scipy.stats.pearsonr`/`spearmanr` (which return p-values directly), plus a 95% CI via Fisher z-transform (`np.arctanh`/`np.tanh`). With n = 1,200, nearly everything comes back statistically significant, so **effect size (Cohen's thresholds: <0.1 negligible, 0.1–0.3 small, 0.3–0.5 moderate, >0.5 large) is what actually distinguishes meaningful variables from noise**, not the p-value.
- **Results**: `total_sessions` is the strongest variable by far (Pearson/Spearman ≈ 0.52/0.55 at 30d, 0.59/0.62 at 90d — large effect, both windows). `team_invite_sent` and `premium_trial_started` are small-to-moderate (~0.25–0.33); `first_project_created` is small (~0.16–0.19); `account_setup_completed` is negligible (~0.01–0.03) despite being "significant" — its 83% completion rate rules out low-variance as the explanation, it simply doesn't track with retention. `team_invite_sent` and `total_sessions` both strengthen from 30d to 90d (team_invite_sent moves small → moderate), while `first_project_created`/`premium_trial_started` stay flat — treated as association, not causation, throughout (an equally valid explanation is that durable-engagement users both invite teammates and generate more sessions, rather than either behavior driving retention).
- **Composite `activation_score`**: unweighted sum of the three activation flags that showed signal (`first_project_created` + `team_invite_sent` + `premium_trial_started`, excluding `account_setup_completed`), giving a 0–3 score per user. `total_sessions` is deliberately excluded from the sum since it's a different scale/skewed and would dominate the composite by magnitude alone. The composite correlates moderately with retention (r ≈ 0.40 at 30d, 0.45 at 90d) — stronger than any single step, weaker than `total_sessions` alone. Retention rate by score is monotonic and steep: 30.5%/16.9% (30d/90d) at score 0 up to 92.2%/89.1% at score 3.
- **Risk segmentation**: three-tier rule combining `activation_score` and `total_sessions` (split at the median, 8, not the mean, since the distribution is skewed) — **High risk** = score 0 and below-median sessions; **High potential** = score ≥2 and at/above-median sessions; **Watch list** = everything else. Validated against actual retention rates: High risk 19.8%/5.9%, Watch list 61.8%/45.5%, High potential 91.0%/86.2% (30d/90d) — a wider spread than `activation_score` alone, confirming the two-signal rule sharpens the split. Thresholds are data-derived from this dataset, not independently validated — a starting rule, not a fitted model.

## Channel Segmentation Analysis Lab

A twelfth lab in `channel_segmentation_analysis/channel_segmentation.ipynb` — acquisition-channel efficiency vs. contribution analysis on a synthetic StreamlineApp dataset (`data/StreamlineApp_ChannelSegmentation_Data.xlsx`, "Raw Data" sheet, 7,390 users across 6 channels, one row per user: `user_id`, `acquisition_channel`, `registration_date`, `activated`).

Key design decisions:

- **Two separate questions, not one ranking**: `activation_rate` (activated_count / user_count per channel) measures pure efficiency independent of channel size; `contribution_index_pts` measures whether a channel's share of total activations outruns its share of total users. Combining these into a quadrant view (rather than ranking channels on a single number) avoids conflating "efficient" with "impactful."
- **`contribution_index_pts = contribution_pct − user_share_pct`** — positive means the channel converts a bigger slice of total activations than its user volume alone would predict (over-indexing); negative means it under-delivers relative to size; zero means contribution is exactly proportional to user share. Algebraically `contribution_index_pts = user_share_pct × (activation_rate / baseline − 1)`, so it's 0 exactly when a channel's `activation_rate` equals the blended baseline — nothing more; a near-zero index for a large-share channel just means it's converting at parity with the average, not that it's "quietly bad" (re-check this caveat if any channel's user share grows toward ~40–50%, since a dominant channel starts to define the baseline itself).
- **Why the y-axis benchmark is user-share-weighted, not a flat mean**: an earlier version of this analysis benchmarked `contribution_pct` against `contribution_pct.mean()` (i.e. `100% / n_channels ≈ 16.67%`), which structurally penalized small channels — a 90-user channel (Partner Referrals) could never reach a 16.67% share of activations even at 100% conversion, so it was doomed into the "low contribution" quadrant regardless of performance. Benchmarking against each channel's own `user_share_pct` (what its contribution *would* be if it converted at the baseline rate) fixes this — see the "Why the quadrant benchmark is user-share-weighted" markdown cell in the notebook for the full derivation.
- **Quadrant chart**: x = `activation_rate` vs. `baseline_activation_rate` (34.11%, blended); y = `contribution_index_pts` vs. 0. Four quadrants — top-right (green, above-baseline efficiency + over-indexing, "double down"), bottom-right (blue, efficient but under-delivering on volume, "room to scale"), top-left (orange, inefficient but still over-indexing — usually just a large channel, "investigate before cutting"), bottom-left (red, below-baseline + under-indexing, "weakest, first candidates to fix or de-prioritize").
- **Result on this dataset**: Organic Search, Direct Traffic, Email Marketing, and Partner Referrals all land top-right (green); Paid Search and Social Media Ads land bottom-left (red), with Social Media Ads the single weakest channel on both axes (25.53% activation rate, −5.11pt index) — flagged as the top root-cause-investigation priority. Partner Referrals has the best raw activation rate (61.11%) but only 90 users, so its index is small in absolute points purely from channel size, not underperformance — treated as "protect and monitor," not a scaling lever.
- **Documented limitations** (not corrected in the analysis): cross-sectional snapshot only, no time trend; no cost/spend data, so "over-indexing on contribution" isn't the same as "profitable" — paid channels (Paid Search, Social Media Ads) carry acquisition costs that free/organic channels don't, understating their urgency relative to similarly-placed free channels; no significance testing on the `activation_rate` gaps between channels.
